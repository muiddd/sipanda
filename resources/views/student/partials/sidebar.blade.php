...
<aside class="hidden lg:flex fixed top-0 left-0 h-screen w-72 p-6 flex-col z-50">
    <div class="relative glass glass-sidebar h-full flex flex-col p-6 border-l-0 overflow-hidden bg-white/70 dark:bg-[#121212]/70 backdrop-blur-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] border-r border-white/20 dark:border-white/10">
        
        <!-- Subtle Top Glow Decoration -->
        <div class="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-40 bg-gradient-to-br from-[#75cb50]/20 to-[#10b981]/20 rounded-full blur-3xl pointer-events-none -z-10"></div>
        ...
        <nav class="flex-1 space-y-6 overflow-y-auto pr-1 scrollbar-hide">
            ...
            <div>
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-3 px-3">
                    Menu Utama
                </p>
                <div class="space-y-1">
        ...
        <div class="flex-shrink-0 mt-4 space-y-4 pt-4 border-t border-black/5 dark:border-white/10">
            ...
            {{-- Slider Theme Toggle --}}
            <div class="relative flex items-center bg-slate-100 dark:bg-white/5 p-1 rounded-2xl border border-black/5 dark:border-white/5">
                <button id="theme-toggle-light-btn" class="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold transition-all duration-300 z-10 text-slate-500 dark:text-slate-400">
                ...
                <button id="theme-toggle-dark-btn" class="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold transition-all duration-300 z-10 text-slate-500 dark:text-slate-400">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                    </svg>
                    <span>Gelap</span>
                </button>
                <div id="theme-indicator" class="absolute top-[4px] bottom-[4px] left-[4px] w-[calc(50%-4px)] bg-white dark:bg-white/10 rounded-xl shadow-md border border-black/5 dark:border-white/5 transition-all duration-300 pointer-events-none"></div>
            </div>

            {{-- User Card Profile --}}
            <div class="p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border border-black/5 dark:border-white/5 flex items-center gap-3 transition hover:shadow-lg hover:border-[#75cb50]/20 duration-300">
                <div class="relative group flex-shrink-0">
                    <div class="absolute -inset-0.5 bg-gradient-to-tr from-[#75cb50] to-[#10b981] rounded-full opacity-60 group-hover:opacity-100 transition duration-300 blur-sm"></div>
                    <div class="relative w-9 h-9 rounded-full bg-slate-900 dark:bg-white flex items-center justify-center font-bold text-white dark:text-slate-900 font-heading text-base">
                        {{ substr(auth()->user()->name, 0, 1) }}
                    </div>
                </div>
                <div class="overflow-hidden flex-1 min-w-0">
                    <div class="font-bold text-sm text-slate-800 dark:text-white truncate leading-tight">{{ auth()->user()->name }}</div>
                    <div class="text-[10px] text-slate-400 dark:text-slate-500 truncate mt-0.5">{{ auth()->user()->email }}</div>
                </div>
...
<header class="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white/75 dark:bg-[#121212]/75 backdrop-blur-2xl border-b border-black/5 dark:border-white/10 z-40 px-6 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
        ...
        <button id="menu-toggle-mobile" class="p-2.5 text-slate-700 dark:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl transition focus:outline-none z-50">
            <svg class="w-6 h-6 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path id="menu-toggle-mobile-icon" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 6h16M4 12h16M4 18h16"></path>
            </svg>
        </button>
    </div>
</header>
...
<div id="mobile-menu-dropdown" class="fixed top-[72px] left-6 right-6 lg:hidden bg-white/95 dark:bg-[#121212]/95 backdrop-blur-3xl border border-black/5 dark:border-white/10 rounded-3xl p-6 shadow-2xl z-[150] transform scale-95 opacity-0 pointer-events-none transition-all duration-300 origin-top flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-4 text-center">
        ...
        <div class="relative flex items-center bg-slate-100 dark:bg-white/5 p-1 rounded-2xl border border-black/5 dark:border-white/5">
            <button id="theme-toggle-light-mobile" class="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold transition-all duration-300 z-10 text-slate-500 dark:text-slate-400">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <span>Terang</span>
            </button>
            <button id="theme-toggle-dark-mobile" class="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold transition-all duration-300 z-10 text-slate-500 dark:text-slate-400">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                </svg>
                <span>Gelap</span>
            </button>
            <div id="theme-indicator-mobile" class="absolute top-[4px] bottom-[4px] left-[4px] w-[calc(50%-4px)] bg-white dark:bg-white/10 rounded-xl shadow-md border border-black/5 dark:border-white/5 transition-all duration-300 pointer-events-none"></div>
        </div>

        <div class="flex items-center gap-3 bg-black/5 dark:bg-white/5 p-3 rounded-2xl">
            <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-[#75cb50] to-[#10b981] flex items-center justify-center font-bold text-white font-heading text-lg shadow-[0_0_15px_rgba(34,197,94,0.4)] flex-shrink-0">
                {{ substr(auth()->user()->name, 0, 1) }}
            </div>
            ...
            <a href="{{ route('student.settings') }}"
                class="flex-shrink-0 text-slate-500 dark:text-slate-400 hover:text-[#75cb50] dark:hover:text-[#75cb50] hover:bg-black/5 dark:hover:bg-white/10 p-2 rounded-lg transition
